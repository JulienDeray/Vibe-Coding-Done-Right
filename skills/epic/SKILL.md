...
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, Task, EnterPlanMode, ExitPlanMode, AskUserQuestion, Skill, mcp__notion__notion-fetch, mcp__notion__notion-search, mcp__notion__notion-update-page, mcp__claude-in-chrome__*
---
...
#### Step 11: Manual Browser Testing
Verify the implemented feature in the browser using Chrome MCP tools.

**Prerequisites:**
- Ensure the API is running: `npm run dev:api`
- Ensure the web UI is running: `npm run dev:web`

**Testing process:**
1. Use Chrome MCP tools to open the application in the browser (http://localhost:5173)
2. Navigate to the implemented feature
3. Test each acceptance criterion from the PRD:
   - Verify expected UI elements are present
   - Test user interactions (clicks, forms, navigation)
   - Confirm data displays correctly
   - Check error states if applicable

4. Use AskUserQuestion to report results:

```json
{
  "questions": [{
    "question": "Browser testing complete. What is the result?",
    "header": "Test Result",
    "options": [
      {"label": "All tests pass", "description": "Feature works as expected, ready for wrap-up"},
      {"label": "Minor issues", "description": "Small bugs found, will fix before continuing"},
      {"label": "Major issues", "description": "Significant problems, need to revisit implementation"}
    ],
    "multiSelect": false
  }]
}
...
- Return to Step 10 to fix the implementation
- Re-run browser testing after fixes

**Guidelines:**
- Test the happy path first, then edge cases
- Compare behavior against PRD acceptance criteria
- Take screenshots of any issues for documentation

**If Claude-in-Chrome MCP is not available:**
If `mcp__claude-in-chrome__*` tools are not available, ask the user to restart Claude with the Chrome extension:

```json
{
  "questions": [{
    "question": "The Claude-in-Chrome MCP is not available. How would you like to proceed?",
    "header": "Chrome MCP",
    "options": [
      {"label": "Restart with --chrome", "description": "Exit, run 'claude --chrome --resume' to enable browser testing"},
      {"label": "Manual testing", "description": "I'll test manually and report results"},
      {"label": "Skip browser testing", "description": "Proceed to wrap-up without browser verification"}
    ],
    "multiSelect": false
  }]
}
...
If user selects "Restart with --chrome":
- Inform them to run: `claude --chrome --resume`
- This will restart Claude with the Chrome extension enabled and resume the current session

### Phase 5: Wrap-up

#### Step 12: Finalize Implementation
...
This will:
- Update Notion checkboxes (if applicable)
- Write tests via /testing-workflow
- Validate via /validate
- Update local documentation in `docs/`
- Commit changes
- Report to PM via /report-to-pm
...
- [ ] Implementation finalized via /wrap-up
